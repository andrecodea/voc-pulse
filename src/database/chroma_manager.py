...
try:
    ...
    print("ERRO: 'config/config.yaml' não encontrado. {e}")
    COLLECTION_NAME = "voc_pulse_default"

# 2. Inicializa o ChromaDB
def initialize_chromadb(df_enriched: pd.DataFrame):
    """
    Initializes local ChromaDB and populates it with the PRE-PROCESSED data from the
    enriched DataFrame

    :param df_enriched: AI Enriched DataFrame.
    :return collection: ChromaDB collection.
    """
    # 1. Cria a coleção
    print("INFO: Inicializando ChromaDB em memória")
    client = chromadb.Client()
    collection = client.get_or_create_collection(name=COLLECTION_NAME)

    # 2. Remove as linhas em que a geração de embeddings falhou
    df_valid = df_enriched[df_enriched['embedding'].apply(lambda x: isinstance(x, list) and len(x)>0)]
